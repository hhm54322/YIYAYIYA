...
- (void)setUrl:(NSString *)url {
  NSURL *strUrl = [NSURL URLWithString:url];//网络视频，填写网络url地址
  [self.aliPlayer prepareWithURL:strUrl];
}
...
-(AliyunVodPlayer *)aliPlayer{
  if (!_aliPlayer) {
    ...
    UIView *playerView = _aliPlayer.playerView;
...
//      }
    ...
    case AliyunVodPlayerEventFinish:
      ...
      if (self.onPlayEndCallback) {
        self.onPlayEndCallback(@{@"type": @"end"});
      }
      break;
    ...
    case AliyunVodPlayerEventEndLoading:
      //视频加载完成时触发
      if (self.onPlayerEventEndLoading) {
        self.onPlayerEventEndLoading(@{@"type": @"endLoading"});
      }
      break;
    case AliyunVodPlayerEventSeekDone:
      //视频Seek完成时触发
      if (self.onPlayerEventSeekDone) {
        self.onPlayerEventSeekDone(@{@"type": @"seekDone"});
      }
      break;
    default:
      break;
    ...
    _timer = [NSTimer scheduledTimerWithTimeInterval:1 repeats:YES block:^(NSTimer * _Nonnull timer) {
      if (selfWeak.aliPlayer) {
        NSDictionary *callbackExt = @{
                                      ...
                                      @"loadedTime": @(selfWeak.aliPlayer.loadedTime)
                                      };
